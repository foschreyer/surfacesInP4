...
nonspecialAlexanderSurface(PolynomialRing,PolynomialRing) := (P4,P2) -> (
    --tenPts = minors(4,random(P2^5,P2^{4:-1}));
...
exampleOfSchreyerSurfaces(Ring) := P4 -> (
    if char P4 !=3 then error "expected coordinate ring of P4 in charcteristic 3";
...
--     PURPOSE : Construct a quintic elliptic scroll, which is a nonsingular surface of degree 5 and sectional genus 1
--       INPUT : 'P4', the homogeneous coordinate ring of projective fourspace 
--      OUTPUT : Ideal of the quintic elliptic scroll
-- DESCRIPTION : This function constructs a quintic elliptic surface as the degeneracy locus of a map between two vector bundles

quinticEllipticScroll(PolynomialRing):=P4 -> (
    -- Compute the koszul complex of the variables of 'P4'
    kos:=res coker vars P4;
    -- Construct a variety 'X' as the degeneracy locus of a map from 5O(-3) to the second Koszul syzygy module 
...
ellipticConicBundle(PolynomialRing) := P4 -> (
    ...
    -- Define a variety 'X' as the degeneracy locus of the map from 4O(-4) to 'K'
...
irregularEllipticSurfaceD12(PolynomialRing) := P4 -> (
    ...
    f:=map(P4^{1:-1},target kos.dd_3,{{1,4:0,-P4_0,2:0,P4_1,0}})*kos.dd_3;
     -- Construct the kernel of 'f'
    K:=prune homology(f,kos.dd_4);
    -- Define a variety 'X' as the degeneracy locus of a map from O(-4)++3O(-5) to 'K'
...
biellipticSurfaceD10(PolynomialRing):=P4->(
    ...
    mu:=1;
    isGood:=false;
    -- Inside the following while loop, find an elliptic curve 'E' containing three ZZ/p-rational nontrivial 2-torsion points and one ZZ/p-rational nontrivial 6-torsion point  
...
biellipticSurfaceD15(PolynomialRing):=P4->(
	...
	if #compP<3 then mu=mu else (
		...
		-- Select the first 6-torsion point and then two 6-torsion point from 'compP'
...
-- DESCRIPTION : This function finds the ideal of an abelian surface as the zero scheme of a global section of the Horrocks-Mumford bundle
--     COMMENT : This function uses the BGG package
...
-- Abelian surface of degree 15, which is linked to the HM abelian surface (B6.2)
...
-- DESCRIPTION : This function constructs the nonminimal abelian surface as the surface residual to the abelian surface of degree 10 in a (5,5) complete intersection
...
--       INPUT : 'P4', the homogeneous coordinate ring of projective fourspace 
--      OUTPUT : Ideal of the abelian surface of degree 15
-- DESCRIPTION : This function constructs a nonminimal abelian surface as the dependency locus of two sections of a rank three vector bundle
...
K3surfaceD7(PolynomialRing):=P4 -> (
    -- Define a variety 'X' as the degeneracy locus of a generic map from O_P4(-1) ++ O_P4(-2) to 3O_P4 
...
K3surfaceD9(PolynomialRing) := P4 -> (
    -- Compute the koszul complex of the variables of 'P4'
    kos:=res coker vars P4;
    -- Define the direct sum 'f' of the fourth exterior power of the cotangent bundle and O_P4(_4)
...
--       INPUT : 'P4', the homogeneous coordinate ring of projective fourspace 
--      OUTPUT : Ideal of the K3 surface of degree 10 with three 6-secant lines
...
K3surfaceD10S9SL3(PolynomialRing) := P4 -> (
    ...
    -- Choose a random map 'f' from 2O_P4(1) ++ 4O_P4 to 2O_P4(2) 
    f:= random(P4^{1:2},P4^{2:1,4:0});
    ff:=res coker f;
    -- Define a random map 'r' from O_P4(-1) ++ the first exterior power of the cotangent bundle twisted by 3 to the direct sum of the first syzygy module of 'f' and O_PA 
...
-- K3 surface of degree 11 and sectional genus 11 with n 6-secant lines (B4.8-11)
...
-- DESCRIPTION : This function constructs the K3 surface as the degeneracy locus of a map between the direct sum of O_P4(-1) and four copies of the third exterior power of the cotangent bundle to
--the direct sum 'F' of three copies of the second exterior power of the cotangent bundle 
...
ellipticSurfaceD9(PolynomialRing) := P4 -> (
    -- Compute the koszul complex of the variables of 'P4'
    kos:=res coker vars P4;
    -- Construct a set of generators for the direct sum of three copies of the cotangent bundle twisted by -1 and two copies of O_P4(-1)
...
Headline => "unirational families of rational surfaces",
   ...
   PARA{},
    ...
    PARA{},
     ...
     UL{
        TO schreyerSurfaces, "    (10 families)",
	TO aboRanestadSurfaces, "     (9 families)",
	}
}
...
Headline => "functions concerning Abo-Ranestad surfaces",
   "[Abo-Ranestad,2005] discovered 4 families of rational surfaces X in P4 with d=12, sectional genus pi=13 via a search over a finite field.
...
Headline => "Known families of K3 surfaces",
   "Various family of non-minmal K3 surfaces are known.
    ...
    their 6-secant behaviours L. Note that a smooth surface in P4 is expected to have 
...
Description
  Text
    In the Tate resolution of an Abo-Ranestad surface their are a 4x2 matrix m4x2 and a 2x3 matrix m2x3
    with linear entries over the exterior algbra. These matrices define rational maps P3 -> G(2,5) and P2 -> G(2,5)
    and the type of the surface depends on how these images intersect in the Grassmannin G(2,5). It turns out that the number of
    (-1) lines on the surface will coincides with the number of intersection points of the images + 1.
  ...
  Text
    ...
    a finite ground field FF_q. Since an extra intersection point is a codimension 1 condition so we can find
...
Description
  ...
  Text
   ...
   betti(T=tateResolutionOfSurface X)
 Text
   The construction uses Moore matrices and a search for a 6 torsion point on an elliptic curve .
...
Description
 ...
 Text
   The construction uses Moore matrices and a search for 6 torsion points on an elliptic curve .
   Perhaps the code can be improved.
Caveat
   The function can fail, in which case it returns null.
References
