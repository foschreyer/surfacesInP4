...
aboRanestadSurface(Ring,Number,Number) := (P4,n,b) -> (
    ...
    (E,m2x3,bs,as,B,ExB,E2,b4x2,a2x3,E3):=prepareAboRanestadSurfaces(P4);
    count:=1;test:=1;
    m2xk:= null; m2x2:=null;I:=null;sol:=null;randSol:=null;m4x2:=null;
    b4x2r:=null;bb:=null;test1:=null;test2:=null;T:=null;X:=null;c:=null;
    while(
    while(
    while(
    ...
    while (
	elapsedTime (X,m4x2)=aboRanestadSurface(P4,n,Smooth=>false);
	singX=X+minors(2,jacobian X);
	dim singX !=0 ) do (countSmooth=countSmooth+1);
    if opt.Verbose=>true then (<<countSmooth <<endl;);
    (X,m4x2))
